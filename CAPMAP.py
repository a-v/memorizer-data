...
class CAPMAP:
    ...
    def __init__(self, vmlinux, kmap_file, verbose=1, import_maps_from = None):
        ...
        # This is an optimization for saving memory:
        # If the maps are the same as from another cmap, just make pointers.
        # Needed for cases where many Capmap objects are created from same vmlinux (overhead experiments).
    ...
    def parse_to_digraph(self, fn):
        ...
        # First check to see if we have compressed form of this capmap
        capmapfile = fn
        ...
        # allows loading in multiple capmaps as long as based on same vmlinux.
        this_dg = nx.DiGraph()
        num_instr = 0
        num_obj = 0
        live_times = []
        ...
        with open(capmapfile, "r") as file:
            ...
            for line in file:
                ...
                if len(myline) == 10:
                    ...
                    if (free_time != 0):
                        live_times.append(free_time - alloc_time)
                    ...
                    if allocator in ["KMALLOC", "KMALLOC_ND", "KMEM_CACHE",
                        ...
                        if "feedbeef" in alloc_ip or "prealloced" in alloc_ip:
                            ...
                            if alloc_ip in self.ip_to_func:
                    ...
                    if not this_dg.has_node(obj):
                        this_dg.add_node(obj, size=size, allocator=allocator,va=va, slab_cache=slab_cache, name=name)
                        num_obj += 1
                    else:
                        ...
                        if memtype == MemType.HEAP or allocator == "VMALLOC":
                            if size > this_dg.nodes[obj]["size"]:
                                this_dg.nodes[obj]["size"] = size
        ...
        if found_compressed == False:
            ...
            for obj_addr in instance_store:
                ...
                # Update object
                this_dg.nodes[obj_node]["size"] = object_size
        ...
        for node in this_dg:
            if node[0] == NodeType.OBJECT:
                ...
                self.capmap_object_sizes[objaddr].append(this_dg.nodes[node]["size"])
    ...
    def load_symbol_table(self):
        ...
        print("WARNING: this implementation currently takes lots of RAM (32GB)")
        ...
        if os.path.isfile(global_table_file):
            ...
            print("Did not find global table. Object metadata will be incomplete.")
        ...
        with open(asm_output) as fh:
            ...
            for line in fh:
                ...
                if funcname != None:
                    ...
                    # Skip finstrument-functions calls.
                    ...
                    '''
                    # Now deprecated:
                    # Assign owners to global objects. Any matching reference
                    # to the obj takes it. This is the current approach because nm command
                    # and DWARF were incomplete. Future upgrade is to explore combining
                    # multiple sources.
                    addrs = addr_extract.findall(line)
                    for found_addr in addrs:
                        found_addr = found_addr[:-1]
                        if found_addr in self.global_objs:
                            self.obj_owner_func[found_addr] = self.ip_to_func[addr]
                            self.obj_owner_file[found_addr] = self.ip_to_file[addr]
                            self.obj_owner_dir[found_addr] = self.ip_to_dir[addr]
                            self.obj_owner_topdir[found_addr] = self.ip_to_topdir[addr]
                    '''
    ...
    def batch_get_info(self, plain, follow_inline=False):
        ...
        for i in range(0,num_batches):
            ...
            for line in output:
                ...
                for field in range(len(line_fields)):
                    line_fields[field] = line_fields[field].decode()
                if (line[0] != ' '):
                    ...
                    if len(line_fields) <= 2:
                        ...
                        if "?" in line_fields[2]:
                            ...
                            if not plain:
                                ...
                                #find the colon in the list
                                for index in range(len(line_fields)):
                                    if ":" in line_fields[index]:
                                        funcname = "".join(self.re_kernel.findall(func))
                                        line_no = line_fields[index].split(":")[1]
        ...
        for i in range(0, min(len(addresses),len(funcs))):
            ip = addresses[i]
    ...
    def add_capmap(self, dg, fn, found_compressed):
        ...
        for node in dg:
            ...
            if not node in self.dg:
                ...
                for prop in ["size", "allocator", "va", "slab_cache", "name"]:
                    if prop in dg.nodes[node]:
                        self.dg.nodes[node][prop] = dg.nodes[node][prop]
                if node[0] == NodeType.OBJECT:
                    added_obj_nodes += 1
                else:
                    added_instr_nodes += 1
            ...
            for objnode in dg.successors(node):
                ...
                if not objnode in self.dg:
                    ...
                    for prop in ["size", "allocator", "va", "slab_cache", "name"]:
                        if prop in dg.nodes[objnode]:
                            self.dg.nodes[objnode][prop] = dg.nodes[objnode][prop]
        ...
        if self.verbose:
            ...
            if len(filename_parts) > 2:
                test_type = filename_parts[0]   
                test_num = filename_parts[1]
                benchname = filename_parts[2].split(".")[0]
                print("\tCONV: " + test_type + " " + test_num + " " + benchname + " " + str(total_added) + "\n")
    ...
    def scale_capmap(self):
        ...
        if os.path.exists(funcpath):
            print("Found funcfile! Going to scale this CAPMAP with " + funcpath)
        else:
            return
        ...
        for node in self.dg:
            if node[0] == NodeType.SUBJECT:
                for obj_node in self.dg.successors(node):
                    ...
                    if edge["call"] > 0:
                        called_func = self.ip_to_func[self.get_node_ip(obj_node)]
                        if ":" in called_func:
    ...
    def make_compressed_capmap(self, dg, fn):
        ...
        for node in dg:
            if node[0] == NodeType.OBJECT:
                ...
                objip = node[2]
                size = dg.nodes[node]["size"]
                allocator = dg.nodes[node]["allocator"]
                va = dg.nodes[node]["va"]
                slab_cache = dg.nodes[node]["slab_cache"]
                if dg.nodes[node]["name"] != None:
                    cfile.write("# " + dg.nodes[node]["name"] +"\n")
    ...
    # TODO Currently skipping this case, could try adding to maps.
    ...
    def build_object_ownership_maps(self):
        ...
        for node in self.dg:
            if node[0] == NodeType.OBJECT:
                ...
                if self.get_node_memtype(node) == MemType.HEAP and this_ip in self.obj_owner_func:
    ...
    def report_stats(self):
        ...
        print("Code size: " + str(total_instr_size))
        ...
        for node in self.dg:
            if node[0] == NodeType.OBJECT:
                ...
                allocator = self.dg.nodes[node]["allocator"]
            ...
            for node in self.dg:
                if node[0] == NodeType.SUBJECT:
                    for obj_node in self.dg.successors(node):
                        ...
                        if edge["call"] > 0:
                            total_calls += edge["call"]
            print("Total calls for " + self.kmap_name + ": " + str(total_calls) + ", cycles/call= " + str(float(self.baseline_cycles) / total_calls))
    ...
    def make_linkmap(self, subj_clusters, obj_clusters, default="mediated", one_unmediated=False):
        ...
        # If one_unmediated=True, each object gets one subject cluster that can access it unmediatedly
        # Calculate that here
        if one_unmediated:

            for node in self.dg:
                if node[0] == NodeType.OBJECT:
                    #print("Adding the one unmediated for " + self.get_node_ip(node))
                    obj_cluster = obj_clusters[self.get_node_ip(node)]
                    candidate_subjs = {}
                    for instr_node in self.dg.predecessors(node):
                        instr_ip = self.get_node_ip(instr_node)
                        instr_subj = subj_clusters[self.ip_to_func[instr_ip]]
                        if not instr_subj in candidate_subjs:
                            candidate_subjs[instr_subj] = 0
                        edge = self.dg.get_edge_data(instr_node, node)
                        for op in ops:
                            candidate_subjs[instr_subj] += edge[op]

                    best_subj_weight = 0
                    best_subj = None
                    for subj in candidate_subjs:
                        if best_subj == None or candidate_subjs[subj] > best_subj_weight:
                            best_subj = subj
                            best_subj_weight = candidate_subjs[subj]
                    if best_subj != None:
                        #print("For obj " + str(self.get_node_ip(node)) + " the best subj was " + best_subj + " with weight " + str(best_subj_weight))
                        for op in ["read", "write", "free"]:
                            linkmap[best_subj][obj_cluster][op] = "Unmediated"
    ...
    def get_node_size(self,node):
        return self.dg.nodes[node]["size"]
